...
class TestUrls(SimpleTestCase):
    ...
    def test_list_url(self):
        url = reverse('producto:product_list')
        self.assertEquals(resolve(url).func, product_list)


    def test_list_by_category_url(self):
        url = reverse('producto:product_list_by_category', args=['any-slug'])
        self.assertEquals(resolve(url).func, product_list)


    def test_detail_url(self):
        url = reverse('producto:product_detail', kwargs={'id': 1, 'slug': 'any-slug'})
